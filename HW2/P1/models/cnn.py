...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for i in range(self.nlayers):
            self.convolutional_layers.append(Conv1d(num_input_channels if i == 0 else num_channels[i-1], num_channels[i],
                                               kernel_sizes[i], strides[i],weight_init_fn=conv_weight_init_fn, bias_init_fn=bias_init_fn))
        self.flatten = Flatten()
        output_width = input_width 
        for width in range (self.nlayers):
            output_width = (output_width - kernel_sizes[width]) // strides[width] + 1
        output_width *= num_channels[-1]
        
        self.linear_layer = Linear(output_width , num_linear_neurons)
    ...
    def forward(self, A):
        ...
        # <---------------------
        for k_layers in range(self.nlayers):
            A= self.convolutional_layers[k_layers].forward(A)
            A = self.activations[k_layers].forward(A)
